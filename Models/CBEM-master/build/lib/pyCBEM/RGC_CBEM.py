...
class CBEM_basic:
    ...
    def setObservations(self, Stimulus : np.ndarray, spkTimes_bins : list[int], window : range) -> None:
        ...
        # Find spike times within the window using numpy (JAX doesn't have in1d)
        window_np = np.array(window)
        spkTimes_np = np.array(spkTimes_bins)
        mask = np.isin(window_np, spkTimes_np)
        self.spkTimes_bins  = jnp.array(np.where(mask)[0]); 
