...
def context(func):
    def wrapper(*args, **kwargs):
        # TensorFlow 2.x doesn't use Sessions - eager execution is default
        # For TF 2.x, we can just run the function directly
        if tf.__version__.startswith('2.'):
            # TF 2.x: No session needed, eager execution by default
            result = func(*args, **kwargs)
        else:
            # TF 1.x: Use session context
            graph = tf.Graph()
