...
HISTORY_SIZE = 40   
BATCH_SIZE = 64
LEARNING_RATE = 1e-4  # <--- שינוי 1: האטנו את הקצב פי 10 ליציבות
EPOCHS = 35           # נתנו לו קצת יותר זמן כי הוא לומד לאט יותר
REGULARIZATION = 1e-5 
...
def train_cnn():
    ...
    print(f"🧠 Model Architecture Created. Input: {dataset.spatial_shape}, History: {HISTORY_SIZE}")
    ...
    plt.title(f"Final Correlation: {test_correlations[-1]:.4f}")
    plt.grid(True)
    plt.tight_layout()
    plt.show()
    
    print(f"\n🏆 FINAL RESULT: Test Correlation = {test_correlations[-1]:.4f}")
